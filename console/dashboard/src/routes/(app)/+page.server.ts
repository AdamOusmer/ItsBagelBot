...
export const load: PageServerLoad = ({ locals }) => {
  ...
  return {
    ...
    // The redesign's live panels. Each read degrades to its own default rather
    // than throwing, so one unreachable backend dims one panel instead of
    // failing the page; the panels then render an honest "not measured" rather
