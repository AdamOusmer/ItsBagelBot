<script lang="ts">
  ...
  // The chart chrome behind the consoles' volume curves: gridlines, the filled
  ...
  // "unavailable" copy stay with the caller, because those are the parts that
  // differ per page while the drawing never does.
  //
  // The path is built here rather than shipped as a fixed `d` string because the
  // point count changes with the window being charted: a 20-minute stream and a
  // 6-hour one both have to fill the same box. Everything is unitless viewBox
  ...
  // event marks never overlap the curve.
  const W = 640;
  const PLOT_TOP = 12;
  ...
  type Geometry = { fill: string; line: string; marks: number[]; last: { x: number; y: number } | null };
  ...
  const geo = $derived.by<Geometry>(() => {
    ...
    // zero and a series quieter than its own peak should still use the full box.
...
>
  <defs>
    <linearGradient id="ovFill-{uid}" x1="0" y1="0" x2="0" y2="1">
  ...
  <path d={geo.fill} fill="url(#ovFill-{uid})" />
