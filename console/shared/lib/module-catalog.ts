...
// The built-in command catalog and the user-facing module catalog: the two
// data-heavy catalogs of the dashboard. Split out of types.ts so the wire/view
// type barrel stays lean; every name is re-exported through types.ts, so all
// existing "@bagel/shared" and "./types" imports keep resolving unchanged.

import type { IconName } from "./icons";
// Type-only back-reference (erased at runtime, no import cycle): permission is
// the dashboard-wide domain enum, same pattern as lib/importer/types.ts.
