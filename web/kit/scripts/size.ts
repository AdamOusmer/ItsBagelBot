...
const ENTRIES: {
  ...
  // "guide try-it evaluator (engine/rehearsal)" budget entry removed
  // 2026-09-22 (docs/specs/variables-catalog.md phase 7 review): the
  // variables guide's "Try it" row it measured (evaluate.ts, wireTryIt) was
  // deleted along with the rest of the search-and-<details> layout when the
  // page became a flat, static table -- there is no more guide bundle that
  // pulls engine/rehearsal.ts in for this, so the budget had nothing left to
  // measure.
];
