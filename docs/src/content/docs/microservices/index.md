...
|---|---|---|---|---|
| [Twitch Ingress](/microservices/twitch-ingress/) | `app/twitch/ingress/` | Elixir (OTP 27+) | EventSub Conduit + WebSocket shards; per-shard supervision; tenant OAuth; filter-and-normalize events | `twitch.ingress.event.*`, `twitch.ingress.status.*`, `twitch.ingress.admin.shards.get` |
...
| [Sesame](/microservices/sesame/) | `app/twitch/sesame/` | Go | Core engine and command processor; consumes ingress events, runs module handlers and commands, routes to outgress | consumes `twitch.ingress.event.*`; publishes to `twitch.outgress.*` |
| [Outgress](/microservices/outgress/) | `app/twitch/outgress/` | Go | Sends to Twitch, YouTube Live Chat, and Discord (REST); per-broadcaster rate limit + daily quota budget (Valkey); channel registry; token lifecycle; kill switch. Discord go-live/offline binds `twitch.ingress.event.stream` directly. | consumes `twitch.outgress.*`, `youtube.outgress.*`, `discord.outgress.*`; serves `bagel.rpc.outgress.*` |
| [Dingress](/microservices/dingress/) | `app/dingress/` | Go | Discord gateway singleton: welcomes, join-to-create voice, tickets, staff logs, slash commands, crumb ranks. Reads the guild reverse index outgress writes. | Discord Gateway WebSocket; `/status` |
...
| [Users](/microservices/users/) | `app/db/users/` | Go | User accounts, status (free/paid/vip), active toggle, Twitch OAuth tokens | `bagel.rpc.dashboard.*`, `bagel.rpc.admin.user.*`, `bagel.rpc.internal.projection.users.get`, `bagel.rpc.internal.tokens.*`; emits `data.users.*` |
| [Commands](/microservices/commands/) | `app/db/commands/` | Go | Custom chat commands | `bagel.rpc.commands.*`, `bagel.rpc.internal.projection.commands.get`; emits `data.commands.changed` |
| [Modules](/microservices/modules/) | `app/db/modules/` | Go | Per-broadcaster feature modules | `bagel.rpc.internal.projection.modules.get`; emits `data.modules.changed` |
| [Notifications](/microservices/notifications/) | `app/db/notifications/` | Go | Dashboard notifications, admin announcements, and TTL cleanup | `bagel.rpc.notifications.*`, `bagel.rpc.admin.notifications.*` |
| [Transactions](/microservices/transactions/) | `app/db/transactions/` | Go | Tebex purchase records | consumes `data.transactions.recorded` |
