...
| [Sesame](/microservices/sesame/) | `app/sesame/` | Go | Core engine and command processor; consumes ingress events, runs module handlers and commands, routes to outgress | consumes `twitch.ingress.event.*`; publishes to `twitch.outgress.*`, `discord.outgress.*` (raid/gift copies; go-live does **not** hop through sesame) |
| [Outgress](/microservices/outgress/) | `app/outgress/` | Go | Sends to Twitch, YouTube Live Chat, and Discord (REST); per-broadcaster rate limit + daily quota budget (Valkey); channel registry; token lifecycle; kill switch. Discord go-live/offline binds `twitch.ingress.event.stream` directly. | consumes `twitch.outgress.*`, `youtube.outgress.*`, `discord.outgress.*`; serves `bagel.rpc.outgress.*` |
