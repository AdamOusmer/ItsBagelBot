...
defmodule Ingress.PublishRuntimeConfigTest do
  ...
  describe "INGRESS_PUBLISH_WIRE" do
    test "the shipped default is the atomic wire" do
      ...
      # The default is written down twice — the env parser above is what a
      # release boots with, Ingress.Config.Publish.wire/0 is what every test and
      # dev run reads when the app env is unset. They disagreed silently before
      # this assertion existed, and a disagreement here means production and the
      # whole test suite exercise different wires.
