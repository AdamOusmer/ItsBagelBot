...
| `INGRESS_PUBLISH_BATCH_INFLIGHT`      | Unresolved atomic batches per shard: a latency × flush-rate window (~1000 cohorts/s × 16 ms of commit-ack latency), not a mirror of the broker's per-stream cap. Cohorts over budget publish individually. | `16`     |
...
which buys per-event granularity back at one RAFT quorum round trip per event.
...
refusing or rejecting a batch and the cohort re-driving per message.
