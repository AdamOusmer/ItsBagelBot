...
type NotificationQuery struct {
	...
	withReads  *NotificationReadQuery
...
func (_q *NotificationQuery) QueryReads() *NotificationReadQuery {
	query := (&NotificationReadClient{config: _q.config}).Query()
...
func (_q *NotificationQuery) WithReads(opts ...func(*NotificationReadQuery)) *NotificationQuery {
	query := (&NotificationReadClient{config: _q.config}).Query()
...
//		GroupBy(notification.FieldScope).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Notification.Query().
//		Select(notification.FieldScope).
//		Scan(ctx, &v)
...
func (_q *NotificationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Notification, error) {
	...
	if query := _q.withReads; query != nil {
		if err := _q.loadReads(ctx, query, nodes,
			func(n *Notification) { n.Edges.Reads = []*NotificationRead{} },
			func(n *Notification, e *NotificationRead) { n.Edges.Reads = append(n.Edges.Reads, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *NotificationQuery) loadReads(ctx context.Context, query *NotificationReadQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *NotificationRead)) error {
	...
	query.withFKs = true
	query.Where(predicate.NotificationRead(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(notification.ReadsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.notification_reads
		if fk == nil {
			return fmt.Errorf(`foreign-key "notification_reads" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "notification_reads" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
