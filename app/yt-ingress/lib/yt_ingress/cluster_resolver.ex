...
defmodule YtIngress.ClusterResolver do
  @moduledoc """
  ...
  on recovery produces duplicate registrations that flap chat session ownership.
  ...
  State lives in `:persistent_term`, written on change only: a
  `:persistent_term.put` triggers a global literal GC, so it must never run on
  every poll, only when the membership set actually differs.
  """
