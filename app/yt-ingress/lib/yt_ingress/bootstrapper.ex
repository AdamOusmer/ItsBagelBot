...
defmodule YtIngress.Bootstrapper do
  @moduledoc """
  Runs on every node and periodically makes sure the cluster-singleton
  watcher is alive somewhere in the cluster:

    * `YtIngress.Watcher` — discovers active broadcasts and owns session
      start/stop.

  Horde's registry guarantees at most one instance; this loop guarantees at
  least one, even right after boot when cluster membership is still syncing,
  or after the node that hosted it disappears.
  ...
  while nothing runs — with the Watcher down, no new stream anywhere in the
  fleet gets discovered. So an `:already_started` singleton is probed for
  process liveness (not a mailbox call, so a singleton busy mid-reconcile is
  ...
  def handle_info(:ensure, state) do
    misses = ensure_singleton(YtIngress.Watcher, "watcher", state.misses)
  ...
  defp ensure_singleton(module, label, misses) do
    case Horde.DynamicSupervisor.start_child(YtIngress.ChatSupervisor, module) do
      {:ok, _pid} ->
  ...
  defp replace_singleton(label, pid) do
    case Horde.DynamicSupervisor.terminate_child(YtIngress.ChatSupervisor, pid) do
      :ok -> :ok
