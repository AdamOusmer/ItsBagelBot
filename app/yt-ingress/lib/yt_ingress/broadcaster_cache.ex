...
defmodule YtIngress.BroadcasterCache do
  @moduledoc """
  In-process read-through cache for broadcaster lane status, so the hot event
  ...
  which loads via `YtIngress.BroadcasterStatus` and caches the answer with a
  ...
  Entries are evicted by `YtIngress.CacheInvalidator` when an invalidation key
  ...
  alias YtIngress.{BroadcasterStatus, Config, Metrics}
  ...
  def init(opts) do
    ...
    {:ok,
     %{
       table: table,
       ttl_ms: Keyword.get(opts, :ttl_ms) || Config.broadcaster_cache_ttl_ms(),
       sweep_interval_ms: sweep_interval_ms,
       loader: Keyword.get(opts, :loader, &BroadcasterStatus.lane_for/1),
       pending_by_id: %{},
       id_by_ref: %{},
  ...
  def handle_call({:invalidate, id}, _from, state) do
    ...
    state =
      if Map.has_key?(state.pending_by_id, id) do
        ...
        task =
          Task.Supervisor.async_nolink(BroadcasterCache.TaskSupervisor, fn ->
            state.loader.(id)
          end)

        Metrics.count("Cache/Loads")
  ...
  def handle_info({:DOWN, ref, :process, _pid, reason}, state) do
    if reason != :normal do
      ...
      Metrics.count("Cache/TaskFailed")
    end
  ...
  defp do_lookup(id, from, state) do
    if Map.has_key?(state.pending_by_id, id) do
      Metrics.count("Cache/LoadsCoalesced")
    ...
    case Map.pop(state.id_by_ref, ref) do
      ...
      {id, id_by_ref} ->
        ...
        if MapSet.member?(state.stale_refs, ref) do
          Metrics.count("Cache/StaleIgnored")
  ...
  defp cache_result(id, {:error, reason}, state) do
    ...
    Metrics.count("Cache/LoadErrors")
