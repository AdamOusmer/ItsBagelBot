...
// The named-counter half of the loyalty repository. Split out of queries.go
// because the two halves share only the ent client: balances are a per-viewer
// ledger with guarded arithmetic, counters are broadcaster-authored key/value
// tallies with their own name and scope grammar. Keeping them in one file made
// that file read as two unrelated APIs.
