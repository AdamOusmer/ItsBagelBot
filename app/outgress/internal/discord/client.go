...
// Package discord is outgress's client for the Discord REST API v10 write
// surface: posting messages into channels as the bot. It is deliberately
// narrow — nothing here reads Discord; the ingress side of a future discord
// gateway service owns that — and every call authenticates with the static
// bot token, which never rotates through outgress: resetting bot credentials
// on the Discord developer portal invalidates the token instantly and takes
// every send down until the env var is updated and the pod restarts.
//
...
func (c *Client) call(ctx context.Context, method, path string, payload []byte) error {
	...
	case res.StatusCode == http.StatusBadRequest:
		return fmt.Errorf("%w: %s", ErrBadRequest, readBody(res))

	case res.StatusCode == http.StatusUnauthorized:
		return ErrAuth

	case res.StatusCode == http.StatusForbidden:
		return fmt.Errorf("%w: %s", ErrForbidden, readBody(res))

	case res.StatusCode == http.StatusNotFound:
		return ErrChannelNotFound

	case res.StatusCode == http.StatusTooManyRequests:
		return &RateLimitError{detail: readBody(res)}

	case res.StatusCode >= 400:
		return fmt.Errorf("discord: api rejected request (%d): %s",
			res.StatusCode, readBody(res))
	}

	return fmt.Errorf("discord: unexpected api status %d", res.StatusCode)
}
