...
func TestModel_Property(t *testing.T) {
	...
	b.Update(now, 1, "pod-a", now, now.Add(time.Hour), rate.Limit(10), 10, rate.Limit(5), 5)
	...
	for i := 0; i < 1000; i++ {
		...
		if action < 10 {
			// 10% chance to renew
			b.Renew(b.Epoch()+1, now.Add(time.Hour))
		} else if action < 15 {
			// 5% chance to resize (same holder)
			b.Update(now, b.Epoch()+1, "pod-a", now, now.Add(time.Hour), rate.Limit(10), 10, rate.Limit(5), 5)
		} else if action < 20 {
			// 5% chance of owner transfer (simulated pod restart/move)
			b.Update(now, b.Epoch()+1, "pod-b", now, now.Add(time.Hour), rate.Limit(10), 10, rate.Limit(5), 5)
		} else if action < 60 {
	...
	// Because every owner transfer drains the bucket, successful admissions will be even lower.
