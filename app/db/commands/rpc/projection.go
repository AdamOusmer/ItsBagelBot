...
// projected command section yet. The user-id guard chain lives in
// projection.ServeProjection, shared with the modules and users services.
func SubscribeProjection(w Wiring, subject string) error {
	return projection.ServeProjection[projection.Request, projection.CommandsReply](w.RPCWiring, subject,
...
func SubscribeFetchProjection(w Wiring, subject string) error {
	return projection.ServeProjection[fetchkeyrpc.FetchListRequest, fetchkeyrpc.FetchListReply](w.RPCWiring, subject,
