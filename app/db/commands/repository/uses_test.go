...
func TestRecordUseReplayAndRangeRollback(t *testing.T) {
	...
	require.NoError(t, repo.RecordUse(ctx, "batch-1", 1001, "!hello", 3))
	require.NoError(t, repo.RecordUse(ctx, "batch-1", 1001, "hello", 3))
	require.Equal(t, int64(3), client.Commands.GetX(ctx, row.ID).Uses)
	require.Error(t, repo.RecordUse(ctx, "batch-1", 1001, "hello", 4))
	require.Equal(t, int64(3), client.Commands.GetX(ctx, row.ID).Uses)
	client.Commands.UpdateOneID(row.ID).SetUses(int64(math.MaxInt64) - 1).SaveX(ctx)
	require.Error(t, repo.RecordUse(ctx, "overflow", 1001, "hello", 2))
	...
	require.NoError(t, repo.RecordUse(ctx, "overflow", 1001, "hello", 1))
	require.Equal(t, int64(math.MaxInt64), client.Commands.GetX(ctx, row.ID).Uses)
	require.Error(t, repo.RecordUse(ctx, "too-big", 1001, "hello", -1))
...
func TestRecordUseMissingCommandDoesNotResurrectOnReplay(t *testing.T) {
	...
	ctx := context.Background()
	defer repo.Close(ctx)
	require.NoError(t, repo.RecordUse(ctx, "missing", 1001, "deleted", 3))
