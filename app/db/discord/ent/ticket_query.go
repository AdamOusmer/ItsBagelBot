...
type TicketQuery struct {
	...
	withTranscript *TicketTranscriptQuery
...
func (_q *TicketQuery) QueryTranscript() *TicketTranscriptQuery {
	query := (&TicketTranscriptClient{config: _q.config}).Query()
...
func (_q *TicketQuery) Clone() *TicketQuery {
	...
	return &TicketQuery{
		...
		withTranscript: _q.withTranscript.Clone(),
...
func (_q *TicketQuery) WithTranscript(opts ...func(*TicketTranscriptQuery)) *TicketQuery {
	query := (&TicketTranscriptClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withTranscript = query
	return _q
}
...
func (_q *TicketQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Ticket, error) {
	...
	if query := _q.withTranscript; query != nil {
		if err := _q.loadTranscript(ctx, query, nodes, nil,
			func(n *Ticket, e *TicketTranscript) { n.Edges.Transcript = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TicketQuery) loadTranscript(ctx context.Context, query *TicketTranscriptQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *TicketTranscript)) error {
	...
	query.withFKs = true
	query.Where(predicate.TicketTranscript(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(ticket.TranscriptColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.ticket_transcript
		if fk == nil {
			return fmt.Errorf(`foreign-key "ticket_transcript" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "ticket_transcript" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
