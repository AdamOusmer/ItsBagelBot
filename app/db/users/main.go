...
func subscribeRPCs(ctx context.Context, wiring rpc.Wiring, client *ent.Client, log *zap.Logger) rpcSubjects {
	...
	svcboot.FatalIf(log, rpc.SubscribeAdmin(wiring, client, s.admin,
		env.Get("NATS_INTERNAL_USERS_GET_SUBJECT", "bagel.rpc.internal.users.get"), invalidationPrefix),
		"failed to subscribe admin rpc")
