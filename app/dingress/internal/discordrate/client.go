...
// rest is the subset of *discordapi.Client both dingress roles call
// through. LimitedClient wraps it so every method pays the shared bucket
// before delegating -- one choke point instead of a call site remembering
// to pay it. It is an interface (rather than *discordapi.Client directly)
// only so tests can inject a recorder without a live Discord endpoint.
type rest interface {
...
// bucket. Both ROLE=gateway (community.Bot.REST) and ROLE=egress
// (the live/setup/clip announcers) send through one of these, so every
// call either role makes -- welcomes, tickets, go-live embeds, guild fills
// -- pays the same fleet-wide token before it reaches Discord.
