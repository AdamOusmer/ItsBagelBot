...
// (chat is perishable; a guild fill can afford to sleep). Before this split,
// dingress WAS that caller and paid nothing: the gateway called the REST
// client directly with no bucket at all. Outgress's chat lanes did pay one,
// through pkg/ratelimit against a fleet-shared Valkey key
// (ratelimit:discord:global; see the deleted app/outgress/internal/worker/
// discord.go's takeDiscordGlobal for the original).
//
// Discord's global limit is per BOT TOKEN, not per process. After this
// split four processes share one token (1 gateway + 3 egress), so the
// bucket has to be shared infrastructure, not a per-process in-memory
// counter -- an in-memory limiter in each of the 4 pods would let the fleet
// send 4x the real budget. Reusing pkg/ratelimit's Valkey-backed token
// bucket (rather than inventing a second limiter) is what makes that
// sharing free: same Spec, same key, same Lua script every other bucket in
