...
func (b *tagsBatcher) post(uuids []string, pending map[string]*batchSlot) {
	...
	// A failed CALL answers every waiter but hydrates nothing: a whole-request
	// failure says nothing about individual players (a 404 here would mean we
	// malformed the request, not that these players don't exist), so their
	// keys stay uncached and the next window retries them. Only a successful
	// response carries per-player truth worth remembering.
	if err != nil {
		for _, u := range uuids {
			deliver(pending[u], batchOutcome{err: err})
		}
		return
	}
	...
	for k, v := range resp.Players {
		// Defensive key normalization: Coral echoes undashed lowercase, but a
		// dashed echo would otherwise strand its player as "missing".
	...
	for _, u := range uuids {
		...
		tags, found := lookup[u]
		if !found {
			// Absent from a successful response = no Coral record for this
			// uuid (present-but-tagless players DO appear, with an empty
			// array). Shape it as the same 404 the individual GET produces
			// for an unknown player, and let StoreCached pin it negative.
			out.err = notFoundErr
		} else {
			out.resp = tagsResponse{UUID: u, Tags: toTagsResponse(tags)}
		}
		deliver(pending[u], out)
		core.StoreCached(ctx, b.p.cache, core.Key(providerName, "playertags", u), tagsTTL, negativeTTL, out.resp, out.err)
	}
}
