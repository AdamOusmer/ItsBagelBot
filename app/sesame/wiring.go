...
func newPipeline(deps engine.Deps, registry *engine.Registry, cfg *config.Config) *engine.Pipeline {
	return engine.NewPipeline(deps, registry, engine.Config{
		...
		// Wiring this was not optional bookkeeping: without it the pipeline
		// dropped every span-derived emote code at the door even with
		// SESAME_AUTOMOD_ADAPTIVE=true, silently disabling the span layer of
		// the caps rescue while Vocab/Baseline (gate-side) kept working - a
		// half-armed dark launch that looked fully armed.
