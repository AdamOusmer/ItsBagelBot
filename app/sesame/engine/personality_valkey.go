...
//   - a per-stream feed counter (personality:feed:<id>);
...
// FeedCount bumps and returns the channel's feed counter. The first feed of the
// window claims the TTL; later feeds leave it in place so the count reads as
// "this stream", not a sliding forever-window.
func (s *ValkeyPersonality) FeedCount(ctx context.Context, broadcasterID uint64) (int64, error) {
	key := personalityKey("feed", broadcasterID)
	...
	if n == 1 {
		...
		if err := s.client.Do(ctx, s.client.B().Expire().Key(key).Seconds(seconds).Build()).Error(); err != nil {
			s.log.Warn("personality: feed expire failed", zap.Uint64("broadcaster_id", broadcasterID), zap.Error(err))
		}
	}
	return n, nil
}
