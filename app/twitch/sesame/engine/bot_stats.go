...
func (s *botStats) count(broadcasterID uint64, isChat bool, n int64) {
	if s == nil {
		return
	}
	s.events.Add(n)
	...
	if broadcasterID != 0 {
		s.countChannel(broadcasterID, isChat, n)
	}
}
...
func (s *botStats) countChannel(broadcasterID uint64, isChat bool, n int64) {
	s.mu.Lock()
	defer s.mu.Unlock()
