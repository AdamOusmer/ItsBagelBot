...
func setupAdminRPCTest(t *testing.T) (*adminRPC, *ent.Client) {
	...
	return &adminRPC{db: client, log: zap.NewNop()}, client
}
...
func TestAdminUserListPagesResults(t *testing.T) {
	...
	first := a.list(ctx, adminRequest{Page: 1, Limit: adminUserPageSize})
	...
	second := a.list(ctx, adminRequest{Page: 2, Limit: adminUserPageSize})
...
func TestAdminUserListSearchesBeforePaging(t *testing.T) {
	...
	reply := a.list(ctx, adminRequest{Page: 1, Limit: adminUserPageSize, Search: "NEEDLE"})
