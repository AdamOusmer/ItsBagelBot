...
func newACLKeyring(t *testing.T, acl *natsacl.ACL) *aclKeyring {
	...
	ring := &aclKeyring{
		operator: operator,
		keys:     &natsacl.Keys{Operator: operatorPub, Accounts: map[string]string{}, Roles: map[string]map[string]string{}},
		accounts: map[string]nkeys.KeyPair{},
		roles:    map[string]map[string]nkeys.KeyPair{},
	}
...
// signedAccountJWTs compiles acl with the keyring's public keys and signs
// every resulting account claim with the operator key.
func signedAccountJWTs(t *testing.T, acl *natsacl.ACL, ring *aclKeyring) map[string]string {
	...
	mintActivations(t, acl, ring, claims)
