...
## Node-side state — NOW OWNED BY THE base ROLE (moved 2026-07-25)

`90-bagelbot-net.conf`, `tailscaled-10-mtu.conf`, `tailscale-udp-gro.sh` and
`tailscale-udp-gro.service` moved to `deploy/ansible/roles/base/files/` and are
applied by the base role on every provision:

- `90-bagelbot-net.conf` → `/etc/sysctl.d/90-bagelbot-net.conf`
- `tailscaled-10-mtu.conf` → `/etc/systemd/system/tailscaled.service.d/10-mtu.conf`
- `tailscale-udp-gro.{sh,service}` → `/usr/local/sbin/` + enabled unit

They used to live here and be applied BY HAND, which is exactly how node4
joined the fleet untuned (2026-07-25) — the same lesson as the untracked k3s
GOMEMLIMIT on node2, one rung further up: tracking a file is not enough if
nothing applies it.
