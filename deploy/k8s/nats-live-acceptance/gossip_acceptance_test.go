...
func TestGossipAcceptanceIsGuardedAndNotReconciled(t *testing.T) {
	kustomization, err := os.ReadFile("../kustomization.yaml")
	require.NoError(t, err)
	require.NotContains(t, string(kustomization), "nats-live-acceptance/gossip")

	cmd := exec.Command("bash", "gossip/local-first.sh")
	cmd.Env = withoutEnv(os.Environ(), "CONFIRM_NATS_GOSSIP_ACCEPTANCE")
...
func TestGossipAcceptanceModelsLocalCoreAndEdges(t *testing.T) {
	topology := readGossipFixture(t, "topology.yaml")
...
func TestGossipAcceptanceConfigsAreCoreNATSWithVerifiedTLS(t *testing.T) {
	common := readGossipFixture(t, "common.conf")
	...
	for _, test := range tests {
		t.Run(test.file, func(t *testing.T) {
			config := readGossipFixture(t, test.file)
...
func TestGossipAcceptanceKeepsEightMillisecondGateAndCleanup(t *testing.T) {
	runner := readGossipFixture(t, "local-first.sh")
...
func readGossipFixture(t *testing.T, name string) string {
	t.Helper()
	data, err := os.ReadFile(filepath.Join("gossip", name))
	require.NoError(t, err)
	return string(data)
}
