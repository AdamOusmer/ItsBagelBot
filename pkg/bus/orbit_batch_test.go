...
func TestAtomicCohortStagesEveryMessageButTheCommitPayload(t *testing.T) {
	...
	for i := range batch {
		if err := <-batch[i].confirmed; err != nil {
			t.Fatalf("message %d of a committed cohort failed: %v", i, err)
		}
	}
...
func TestAtomicOverlapResolvesEachCohortOnItsOwnCommit(t *testing.T) {
	...
	for i := range second {
		if err := <-second[i].confirmed; err != nil {
			t.Fatalf("message %d of the second cohort failed: %v", i, err)
		}
	}
	select {
	...
	worker.acks.Wait()
	for i := range first {
		if err := <-first[i].confirmed; err != nil {
			t.Fatalf("message %d of the first cohort failed: %v", i, err)
		}
	}
...
func TestAtomicOverlapDoesNotReplayAnAmbiguousCommitFailure(t *testing.T) {
	...
	for i := range batch {
		if err := <-batch[i].confirmed; !errors.Is(err, want) {
			t.Fatalf("message %d = %v, want the ambiguous cause %v", i, err, want)
		}
	}
...
func TestAtomicOverlapResolvesAStagingFailureWithoutCommitting(t *testing.T) {
	...
	for i := range batch {
		if err := <-batch[i].confirmed; !errors.Is(err, want) {
			t.Fatalf("message %d = %v, want the staging cause %v", i, err, want)
		}
	}
...
func TestAtomicOverlapDrainsInFlightCommitsOnStop(t *testing.T) {
	...
	for i := range batch {
		if err := <-batch[i].confirmed; err != nil {
			t.Fatalf("message %d of a drained cohort failed: %v", i, err)
		}
	}
}
