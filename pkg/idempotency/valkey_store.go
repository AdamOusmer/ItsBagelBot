...
func (s *ValkeyStore) Seen(ctx context.Context, key string, ttl time.Duration) (bool, error) {
	...
	cmd := s.client.B().Set().Key(full).Value(claimValue).Nx().PxMilliseconds(millis(ttl)).Build()
...
func (s *ValkeyStore) noteFailOpen(key string, err error) {
	n := s.failOpen.Add(1)
	if n == 1 || n%1000 == 0 {
		s.log.Warn("idempotency: valkey claim failed; admitting event (fail-open)",
			zap.String("key", key), zap.Int64("fail_open_total", n), zap.Error(err))
	}
}
