...
func TestKeyLabelAndValue(t *testing.T) {
	t.Run("label", func(t *testing.T) {
		runKeyRuleCases(t, KeyLabel, []keyRuleCase{
	...
	t.Run("value", func(t *testing.T) {
		runKeyRuleCases(t, KeyValue, []keyRuleCase{
...
func runKeyRuleCases(t *testing.T, rule func(string) error, cases []keyRuleCase) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if err := rule(tc.in); !errors.Is(err, tc.want) {
				t.Fatalf("rule(%q) = %v, want %v", tc.in, err, tc.want)
			}
		})
	}
}
